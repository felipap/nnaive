...
var Board, Bot, Circle, Drawable, FixedPole, Food, GeneticEngine, NeuralNet, Neuron, NeuronLayer, Square, Triangle, dist, dist2, genEng, generationCounter, mod, painter, parameters, sigmoid, tic, _Bot, _ref, _ref1, _ref2, _ref3,
...
GeneticEngine = (function() {
  var avgFitness, bestFitness, bestGenoma, crossover, crossoverRate, genCounter, mutate, mutationRate, population, totalFitness, worstFitness;
  ...
  crossover = function(mums, dad) {
    var baby1, baby2, cp, i, _i, _j, _ref4;
    if (mums === dads || parameters.crossoverRate < Math.random()) {
      return [mums.slice(0), dad.slice(0)];
    }
    baby1 = [];
    baby2 = [];
    ...
    for (i = _j = cp, _ref4 = mums.length; cp <= _ref4 ? _j <= _ref4 : _j >= _ref4; i = cp <= _ref4 ? ++_j : --_j) {
      baby1.push(dads[i]);
      baby2.push(mums[i]);
    }
    return [baby1, babby2];
  ...
  GeneticEngine.prototype.getChromoRoulette = function(population) {
    ...
    slice = Math.random() * _.reduce(population, function(a, b) {
  ...
  GeneticEngine.prototype.epoch = function(oldpop) {
    ...
    while (newpop.length < parameters.popSize) {
      ...
      _ref5 = crossover(mum.weights, dad.weights), baby1 = _ref5[0], baby2 = _ref5[1];
      mutate(baby1);
      mutate(baby2);
